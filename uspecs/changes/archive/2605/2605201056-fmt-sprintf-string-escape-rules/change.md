...
- [x] update: violating `fmt.Sprintf`/`fmt.Errorf` call sites found by the audit to conform to the new guide
  ...
  - fix: also rewrote multiple JSON-body builder / structured-string call sites to use `json.Marshal` and related safe escaping per the new guide, including additional updates in areas such as router, coreutils, sys, cluster, and workspace code touched by this PR
