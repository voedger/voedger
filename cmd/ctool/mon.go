...
func setMonPassword(cluster *clusterType, password string) error {
	...
	return setGrafanaRandomAdminPassword(cluster)
}
...
func setGrafanaRandomAdminPassword(cluster *clusterType) error {
	return setGrafanaAdminPassword(cluster, randomPassword(minMonPasswordLength))
}
...
func addGrafanUser(node *nodeType, password string) error {
	...
	return newScriptExecuter(node.cluster.sshKey, "").
		run("g-ds-update.sh", node.address(), admin, admin, password)
}
