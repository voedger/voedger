...
service_name=$(yq e '.services | to_entries | .[] | select(.value.healthcheck.test[] | contains("'$lost_node'")) | .key' docker-compose.yml)

if [ -z "$service_name" ]; then
  echo "Could not find a service with IP address $lost_node"
  exit 1
fi
