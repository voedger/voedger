...
for host in "$@"; do
    ...
    if [ -z "$APP_NODE_CONTAINER" ]; then
        ...
        utils_ssh ${SSH_USER}@${host} "sed -i 's/admin:.*/admin: ${HASHED_PASSWORD//\//\\/}/' ~/prometheus/web.yml"
        echo "Password for admin user in Prometheus on ${host} was successfully changed"
