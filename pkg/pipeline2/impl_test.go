...
func TestBasicUsage_SyncPipeline(t *testing.T) {
	...
	pipeline := NewSyncPipeline(context.Background(), "noname",
		WireFunc[any]("Set name", funcOpSetName),
		WireFunc[any]("Set age", funcOpSetAge),
...
func TestBasicUsage_AsyncSwitchOperator(t *testing.T) {
	...
	switchLogic := mockSwitch[IWorkpiece]{
		func(work IWorkpiece) (branchName string, err error) { return targetBranchName, nil },
	}
...
func TestBasicUsage_SwitchOperator(t *testing.T) {
	...
	squarer := mockSyncOp().
		...
		create()
	switchOperator := SwitchOperator[any](switchLogic,
		SwitchBranch[any]("splitter", splitter),
		SwitchBranch[any]("squarer", squarer))
...
func TestBasicUsage_ServiceOperator(t *testing.T) {
	...
	p := NewSyncPipeline(ctx, "testPipeline",
		WireSyncOperator("service1", ServiceOperator[any](service1)),
		WireSyncOperator("service2", ServiceOperator[any](service2)),
	)
