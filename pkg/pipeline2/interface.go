...
type ISyncOperator[T any] interface {
	IOperator
	// If `err` is not nil then `work` is passed to the nearest `ICatch`
	DoSync(ctx context.Context, work T) (err error)
}
...
type ISyncPipeline[T any] interface {
	IPipeline
	ISyncOperator[T]
	SendSync(work T) (err error)
}
...
type ISwitch[T any] interface {
	Switch(work T) (branchName string, err error)
}

type Fork[T any] func(work T, branchNumber int) (fork T, err error)

func ForkSame[T any](work T, _ int) (fork T, err error) { return work, nil }
