...
func TestBundledHostState_BasicUsage(t *testing.T) {
	...
	aaState := factory(context.Background(), mockedAppStructs, nil, state.SimpleWSIDFunc(istructs.WSID(1)), n10nFn, nil, nil, nil, nil, 2, 1, state.NullOpts, nil, nil)
...
func TestAsyncActualizerState_Read(t *testing.T) {
	t.Run("Should flush bundle before read", func(t *testing.T) {
		...
		s := ProvideAsyncActualizerStateFactory()(context.Background(), func() istructs.IAppStructs { return mockedStructs }, nil, state.SimpleWSIDFunc(istructs.WSID(1)), nil, nil, nil, nil, nil, 10, 10, state.NullOpts, nil, nil)
...
func asyncActualizerStateWithTestStateStorage(s *mockStorage) istructs.IState {
	as := ProvideAsyncActualizerStateFactory()(context.Background(), nilAppStructsFunc, nil, nil, nil, nil, nil, nil, nil, 10, 10, state.NullOpts, nil, nil)
