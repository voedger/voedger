...
func implProvideSyncActualizerState(vvmCtx context.Context, appStructsFunc state.AppStructsFunc, partitionIDFunc state.PartitionIDFunc,
	...
	hs := &syncActualizerState{
		hostState: newHostState(vvmCtx, "SyncActualizer", intentsLimit, appStructsFunc),
		eventFunc: eventFunc,
	}
	ieventsFunc := func() istructs.IEvents {
		return appStructsFunc().Events()
	}
	hs.addStorage(sys.Storage_View, storages.NewViewRecordsStorage(vvmCtx, appStructsFunc, wsidFunc, n10nFunc), S_GET|S_GET_BATCH|S_INSERT|S_UPDATE)
	hs.addStorage(sys.Storage_Record, storages.NewRecordsStorage(appStructsFunc, wsidFunc, nil), S_GET|S_GET_BATCH)
	hs.addStorage(sys.Storage_WLog, storages.NewWLogStorage(vvmCtx, ieventsFunc, wsidFunc), S_GET)
