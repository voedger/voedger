...
func TestRecordsStorage_GetBatch(t *testing.T) {
	...
	t.Run("Should handle general records", func(t *testing.T) {
		...
		k1, err := s.KeyBuilder(RecordsStorage, istructs.NullQName)
		require.NoError(err)
		k1.PutRecordID(Field_ID, 1)
		k2, err := s.KeyBuilder(RecordsStorage, istructs.NullQName)
		require.NoError(err)
		k2.PutRecordID(Field_ID, 2)
		k2.PutInt64(Field_WSID, 2)
		k3, err := s.KeyBuilder(RecordsStorage, istructs.NullQName)
		require.NoError(err)
	...
	t.Run("Should return error when 'id' not found", func(t *testing.T) {
		...
		k, err := s.KeyBuilder(RecordsStorage, istructs.NullQName)
		require.NoError(err)
