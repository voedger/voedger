...
func TestRecordsStorage_GetBatch(t *testing.T) {
	...
	t.Run("Should handle general records", func(t *testing.T) {
		...
		s := ProvideQueryProcessorStateFactory()(context.Background(), appStructsFunc(appStructs), nil, SimpleWSIDFunc(istructs.WSID(1)), nil, nil, nil, nil, nil, nil)
	...
	t.Run("Should return error when 'id' not found", func(t *testing.T) {
		require := require.New(t)
		s := ProvideQueryProcessorStateFactory()(context.Background(), nilAppStructsFunc, nil, SimpleWSIDFunc(istructs.WSID(1)), nil, nil, nil, nil, nil, nil)
