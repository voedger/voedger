...
func TestAsyncActualizerState_BasicUsage_Old(t *testing.T) {
	...
	appStructs.
		...
		On("Events").Return(&nilEvents{}).
		On("Records").Return(&nilRecords{})
	s := ProvideAsyncActualizerStateFactory()(context.Background(), func() istructs.IAppStructs { return appStructs }, nil, SimpleWSIDFunc(istructs.WSID(1)), n10nFn, nil, nil, 2, 1)
...
func TestAsyncActualizerState_Read(t *testing.T) {
	t.Run("Should flush bundle before read", func(t *testing.T) {
		...
		appStructs.
			...
			On("Records").Return(&nilRecords{}).
			On("Events").Return(&nilEvents{})
		s := ProvideAsyncActualizerStateFactory()(context.Background(), func() istructs.IAppStructs { return appStructs }, nil, SimpleWSIDFunc(istructs.WSID(1)), nil, nil, nil, 10, 10)
