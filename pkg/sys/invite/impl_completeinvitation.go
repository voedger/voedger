...
func provideCmdCompleteInvitation(sr istructsmem.IStatelessResources) {
	sr.AddCommands(appdef.SysPackagePath, istructsmem.NewCommandFunction(
		qNameCmdCompleteInvitation,
		execCmdCompleteInvitation,
	))
}

func execCmdCompleteInvitation(args istructs.ExecCommandArgs) (err error) {
	...
	svbCDocInvite.PutInt32(Field_State, int32(State_Invited))
	svbCDocInvite.PutString(field_VerificationCode, args.ArgumentObject.AsString(field_VerificationCode))
