...
func parseWSTemplateBLOBs(fsEntries []fs.DirEntry, blobIDs map[int64]map[string]struct{}, wsTemplateFS utils.EmbedFS) (blobs []BLOB, err error) {
...
func ValidateTemplate(wsTemplateName string, epWSTemplates vvm.IEPWSTemplates, wsKind appdef.QName) (wsBLOBs []BLOB, wsData []map[string]interface{}, err error) {
	...
	wsTemplateFS := wsTemplateFSIntf.(utils.EmbedFS)
