...
func TestRates_BasicUsage(t *testing.T) {
	...
	for i := 0; i < 2; i++ {
		vit.PostWS(ws, "q.app1.RatedQry", bodyQry)
		vit.PostWS(ws, "c.app1.RatedCmd", bodyCmd)
	}

	// 3rd is failed because per-minute rate is exceeded
	vit.PostWS(ws, "q.app1.RatedQry", bodyQry, coreutils.Expect429())
	vit.PostWS(ws, "c.app1.RatedCmd", bodyCmd, coreutils.Expect429())
