...
func TestN10NSubscribeErrors(t *testing.T) {
	...
	t.Run("bad requests", func(t *testing.T) {
		cases := []struct {
			...
			{`{"subscriptions":[{"entity":42,"wsid":42}]}`, `cannot unmarshal number into Go struct field`},
			{`{"subscriptions":[{"entity":42,"wsid":"str"}]}`, `cannot unmarshal number into Go struct field`},
			{`{"subscriptions":[{"entity":"test.test","wsid":"str"}]}`, `cannot unmarshal string "str" into Go value of type json.Number`},
