...
func TestForeignAuthorization(t *testing.T) {
	...
	body := fmt.Sprintf(`{"args": {"WSName": "%s","WSKind": "simpleApp.WSKind","WSKindInitializationData": "{\"IntFld\": 10}","TemplateName": "test_template","WSClusterID": 42}}`, wsName)
	vit.PostWS(parentWS, "c.sys.InitChildWorkspace", body)
	...
	t.Run("subjects", func(t *testing.T) {
		...
		cudBody := `{"cuds": [{"fields": {"sys.ID": 1,"sys.QName": "simpleApp.articles","name": "cola","article_manual": 1,"article_hash": 2,"hideonhold": 3,"time_active": 4,"control_active": 5}}]}`
	...
	t.Run("API token", func(t *testing.T) {
		...
		body = `{"args":{"Schema":"simpleApp.articles"},"elements":[{"fields":["sys.ID"]}]}`
