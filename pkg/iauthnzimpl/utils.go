...
func matchOrNoSpecified_Fields(fieldsPattern []string, toFind []string) bool {
	if len(fieldsPattern) == 0 {
		return true
	}
	for _, field := range fieldsPattern {
		if !slices.Contains(toFind, field) 
	}
}
