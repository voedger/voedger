...
var TestSubjectRolesGetter = func(context.Context, string, istructs.IAppStructs, istructs.WSID) ([]schemas.QName, error) {
	return nil, nil
}

func IssueAPIToken(appTokens istructs.IAppTokens, duration time.Duration, roles []schemas.QName, wsid istructs.WSID, currentPrincipalPayload payloads.PrincipalPayload) (token string, err error) {
		...
		for _, prnAND := range prnsAND {
			...
			for _, actualPrn := range actualPrns {
				...
				isMatched = func() bool {
					...
					if prnAND.QName != schemas.NullQName && prnAND.QName != actualPrn.QName {
						return false
					}
...
func matchOrNotSpecified_QNames(arr []schemas.QName, toFind schemas.QName) bool {
	return len(arr) == 0 || slices.Contains(arr, toFind)
}
...
func prnsToString(prns []iauthnz.Principal) string {
	...
	for i := 0; i < len(prns); i++ {
		...
		if prn.QName != schemas.NullQName {
