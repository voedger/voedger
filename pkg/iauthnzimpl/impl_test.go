...
func TestBasicUsage(t *testing.T) {
	...
	appStructs := AppStructsWithTestStorage(map[istructs.WSID]map[istructs.QName]map[istructs.RecordID]map[string]interface{}{
	...
	t.Run("authorize", func(t *testing.T) {
		...
		authzReq := iauthnz.AuthzRequest{
			...
			Resource:      istructs.NewQName(istructs.SysPackage, "SomeCmd"),
		}
...
func TestAuthenticate(t *testing.T) {
	...
	testRole := istructs.NewQName(istructs.SysPackage, "test")
	apiKeyToken, err := IssueAPIToken(appTokens, time.Hour, []istructs.QName{
		testRole,
	}, 2, pp)
	require.NoError(err)
	...
	qNameCDocComputers := istructs.NewQName("untill", "computers")
	...
	appStructs := AppStructsWithTestStorage(map[istructs.WSID]map[istructs.QName]map[istructs.RecordID]map[string]interface{}{
		...
		istructs.WSID(1): {
			...
			qNameCDocComputers: {
				2: {
					istructs.SystemField_QName:    qNameCDocComputers,
					istructs.SystemField_IsActive: true,
				},
				5: {
					istructs.SystemField_QName:    qNameCDocComputers,
					istructs.SystemField_IsActive: false,
				},
			},
			// not used for authorization, but keep for an example
			istructs.NewQName("untill", "restaurant_computers"): {
				3: {},
				6: {},
			},
		},
	...
	testCases := []struct {
		...
		},
		{
			...
			subjects: []istructs.QName{qNameRoleResellersAdmin},
		},
		{
			...
			subjects: []istructs.QName{qNameRoleUntillPaymentsReseller},
		},
		{
	...
	var subjects *[]istructs.QName
	subjectsGetter := func(context.Context, string, istructs.IAppStructs, istructs.WSID) ([]istructs.QName, error) {
		return *subjects, nil
	}
...
func TestAuthorize(t *testing.T) {
	...
	testCmd := istructs.NewQName(istructs.SysPackage, "testcmd")
	testCases := []struct {
		desc     string
...
func TestACLAllow(t *testing.T) {
	defer logger.SetLogLevel(logger.LogLevelInfo)
	require := require.New(t)
	testQName1 := istructs.NewQName(istructs.SysPackage, "testQName")
			...
			acl: ACL{
				{
					desc: "allow rule",
					pattern: PatternType{
						qNamesPattern:  []istructs.QName{testQName1},
					...
					prns: [][]iauthnz.Principal{
						{
							...
							},
							{
								...
								QName: istructs.NewQName(istructs.SysPackage, "testGroup"),
							},
							{
			...
			acl: ACL{
				{
					...
					pattern: PatternType{
						qNamesPattern: []istructs.QName{qNameCmdCreateUPProfile},
						principalsPattern: [][]iauthnz.Principal{
							// OR
...
func TestErrors(t *testing.T) {
	...
	t.Run("personal access token for a system role", func(t *testing.T) {
		for _, sysRole := range iauthnz.SysRoles {
			token, err := IssueAPIToken(appTokens, time.Hour, []istructs.QName{sysRole}, 1, payloads.PrincipalPayload{})
	...
	t.Run("personal access token for NullWSID", func(t *testing.T) {
		token, err := IssueAPIToken(appTokens, time.Hour, []istructs.QName{istructs.NewQName(istructs.SysPackage, "test")}, istructs.NullWSID, payloads.PrincipalPayload{})
...
func AppStructsWithTestStorage(data map[istructs.WSID]map[istructs.QName]map[istructs.RecordID]map[string]interface{}) istructs.IAppStructs {
...
func (as *implIAppStructs) Schemas() istructs.ISchemas          { panic("") }
func (as *implIAppStructs) ClusterAppID() istructs.ClusterAppID { panic("") }
func (as *implIAppStructs) AppQName() istructs.AppQName         { panic("") }
func (as *implIAppStructs) IsFunctionRateLimitsExceeded(istructs.QName, istructs.WSID) bool {
	panic("")
}
...
type implIRecords struct {
	data map[istructs.WSID]map[istructs.QName]map[istructs.RecordID]map[string]interface{}
}
...
func (r *implIRecords) GetSingleton(wsid istructs.WSID, qName istructs.QName) (record istructs.IRecord, err error) {
	if wsData, ok := r.data[wsid]; ok {
...
func (r *implIRecord) QName() istructs.QName     { return r.qName }
func (r *implIRecord) ID() istructs.RecordID     { return r.AsRecordID(istructs.SystemField_ID) }
