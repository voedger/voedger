...
func BuildAppWorkspaces(vvm *HVM, hvmConfig *HVMConfig) error {
	for _, appQName := range vvm.HVMApps {
		...
		for wsNum := 0; istructs.AppWSAmount(wsNum) < as.WSAmount(); wsNum++ {
			...
			partition := istructs.PartitionID(appWSID % istructs.WSID(hvmConfig.PartitionsCount))
			...
			grebp := istructs.GenericRawEventBuilderParams{
				...
				RegisteredAt:      istructs.UnixMilli(hvmConfig.TimeFunc().UnixMilli()),
			...
			reb := as.Events().GetSyncRawEventBuilder(
				istructs.SyncRawEventBuilderParams{
					...
					SyncedAt:                     istructs.UnixMilli(hvmConfig.TimeFunc().UnixMilli()),
				},
			)
			...
			cdocWSDesc.PutInt64(authnz.Field_СreatedAtMs, hvmConfig.TimeFunc().UnixMilli())
			cdocWSDesc.PutInt64(commandprocessor.Field_InitCompletedAtMs, hvmConfig.TimeFunc().UnixMilli())
