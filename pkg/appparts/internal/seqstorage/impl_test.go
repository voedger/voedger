...
func TestReadWrite(t *testing.T) {
	...
	wsb := appDefBuilder.AddWorkspace(testWSQName)
	wsb.AddCDoc(testCDocQName).AddField("IntFld", appdef.DataKind_int32, false)
	wsb.SetDescriptor(testCDocQName)
...
func setupTestAppDef(t *testing.T) appdef.IAppDef {
	...
	appDefBuilder := builder.New()
	wsb := appDefBuilder.AddWorkspace(testWSQName)
	wsb.AddCRecord(testCRecordQName)
	wsb.AddORecord(testORecordQName)
	wsb.AddWRecord(testWRecordQName)
	wsb.AddCDoc(testCDocQName).AddContainer("crecord", testCRecordQName, appdef.Occurs_Unbounded, appdef.Occurs_Unbounded)
	wsb.AddODoc(testODocQName).AddContainer("orecord", testORecordQName, appdef.Occurs_Unbounded, appdef.Occurs_Unbounded)
	wsb.AddWDoc(testWDocQName).AddContainer("wrecord", testWRecordQName, appdef.Occurs_Unbounded, appdef.Occurs_Unbounded)
	wsb.AddCommand(testCmdQName)
	wsb.SetDescriptor(testCDocQName)
	appDef, err := appDefBuilder.Build()
	require.NoError(err)
	return appDef
