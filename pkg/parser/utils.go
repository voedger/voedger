...
func readFileSchemaAST(dir string, fsi embed.FS) (fileSchemasAST []*FileSchemaAST, err error) {
	...
	for _, dirEntry := range dirEntries {
		...
		fileSchemaAST, err := ParseFile(fullFilePath, string(sqlContent))
		if err != nil {
			return nil, err
		}
...
func ReadPackageSchemaAST(ep extensionpoints.IExtensionPoint) (packageSchemaASTs []*PackageSchemaAST, err error) {
	epSqlFiles := ep.ExtensionPoint(apps.EPSqlFiles)
	epSqlFiles.Iterate(func(eKey extensionpoints.EKey, value interface{}) {
		filesSchemasASTs := make([]*FileSchemaAST, 0)
		...
		packageSchemaAST, err := BuildPackageSchema(qualifiedPackageName, filesSchemasASTs)
		if err != nil {
			panic(err)
		}
