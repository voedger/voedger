...
func compile(dir string) (*Result, error) {
	var errs []error
	unknownDeps := make(map[string]struct{})

	loadedPkgs, err := loadPackages(dir, unknownDeps)
	if err != nil {
		return nil, err
	}
	...
	sysPackageAst, compileSysErrs, isSysDir := compileSysPackage(dir, loadedPkgs, importedStmts, pkgFiles, unknownDeps)
	pkgs = append(pkgs, sysPackageAst...)
	errs = append(errs, compileSysErrs...)

	if !isSysDir {
		// compile working dir after sys package
		compileDirPackageAst, compileDirErrs := compileDir(loadedPkgs, dir, loadedPkgs.packagePath, nil, importedStmts, pkgFiles, unknownDeps)
	...
	result := &Result{
		...
		PkgFiles:    pkgFiles,
		UnknownDeps: maps.Keys(unknownDeps),
	}
...
func compileSysPackage(dir string, loadedPkgs *loadedPackages, importedStmts map[string]parser.ImportStmt, pkgFiles map[string][]string, unknownDeps map[string]struct{}) (pkgAsts []*parser.PackageSchemaAST, errs []error, isSysDir bool) {
	if loadedPkgs.modulePath == VoedgerPath {
		...
		pkgAsts, errs = compileDir(loadedPkgs, sysPkgDir, appdef.SysPackage, nil, importedStmts, pkgFiles, unknownDeps)
		return
	}
	pkgAsts, errs = compileDependency(loadedPkgs, appdef.SysPackage, nil, importedStmts, pkgFiles, unknownDeps)
	return
}
...
func compileDir(loadedPkgs *loadedPackages, dir, packagePath string, alias *parser.Ident, importedStmts map[string]parser.ImportStmt, pkgFiles map[string][]string, unknownDeps map[string]struct{}) (packages []*parser.PackageSchemaAST, errs []error) {
	...
	if packageAst != nil {
		importedPackages, compileDepErrs = compileDependencies(loadedPkgs, packageAst.Ast.Imports, importedStmts, pkgFiles, unknownDeps)
...
func compileDependencies(loadedPkgs *loadedPackages, imports []parser.ImportStmt, importedStmts map[string]parser.ImportStmt, pkgFiles map[string][]string, unknownDeps map[string]struct{}) (packages []*parser.PackageSchemaAST, errs []error) {
	for _, imp := range imports {
		dependentPackages, compileDepErrs := compileDependency(loadedPkgs, imp.Name, imp.Alias, importedStmts, pkgFiles, unknownDeps)
		errs = append(errs, compileDepErrs...)
...
func compileDependency(loadedPkgs *loadedPackages, depURL string, alias *parser.Ident, importedStmts map[string]parser.ImportStmt, pkgFiles map[string][]string, unknownDeps map[string]struct{}) (packages []*parser.PackageSchemaAST, errs []error) {
	// workaround for sys package
	depURLToFind := depURL
	if depURL == appdef.SysPackage {
		depURLToFind = sys.PackagePath
	}
	path, err := localPath(loadedPkgs, depURLToFind, unknownDeps)
	if err != nil {
		errs = append(errs, err)
	}
	if logger.IsVerbose() {
	...
	packages, compileDirErrs = compileDir(loadedPkgs, path, depURL, alias, importedStmts, pkgFiles, unknownDeps)
	errs = append(errs, compileDirErrs...)
	return
}

func loadPackages(dir string, unknownDeps map[string]struct{}) (*loadedPackages, error) {
	cfg := &packages.Config{
...
func localPath(loadedPkgs *loadedPackages, depURL string, unknownDeps map[string]struct{}) (localDepPath string, err error) {
	if logger.IsVerbose() {
