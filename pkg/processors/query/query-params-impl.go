...
func newQueryParams(data coreutils.MapObject, elementFactory ElementFactory, filterFactory FilterFactory, orderByFactory OrderByFactory, rootSchema coreutils.SchemaFields) (res IQueryParams, err error) {
	qp := queryParams{}
	...
	return qp, qp.validate(rootSchema)
}
...
func (p queryParams) validate(rootSchema coreutils.SchemaFields) (err error) {
	...
	for _, e := range p.elements {
		...
		for _, field := range e.ResultFields() {
			if _, ok := rootSchema[field.Field()]; !ok {
